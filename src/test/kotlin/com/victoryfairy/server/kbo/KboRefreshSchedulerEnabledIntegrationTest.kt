...
@TestPropertySource(
    properties = [
        "spring.datasource.url=jdbc:h2:mem:kbo-refresh-scheduler-enabled;MODE=PostgreSQL;DB_CLOSE_DELAY=-1",
        "spring.jpa.hibernate.ddl-auto=create-drop",
        "victory-fairy.kbo.refresh.enabled=true",
        "victory-fairy.kbo.refresh.season=2026",
        "victory-fairy.kbo.refresh.cron=0 0 3,9,15,21 * * *",
    ],
)
class KboRefreshSchedulerEnabledIntegrationTest {
    @Autowired lateinit var applicationContext: ApplicationContext
    @Autowired lateinit var scheduler: KboRefreshScheduler
    ...
    @Test
    fun `scheduler runs only when KBO refresh enabled`() {
        assertTrue(applicationContext.getBeansOfType(KboRefreshScheduler::class.java).isNotEmpty())
        stubSchedulePages()
